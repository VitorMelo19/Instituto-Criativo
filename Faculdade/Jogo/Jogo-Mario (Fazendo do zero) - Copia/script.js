document.addEventListener('DOMContentLoaded', () => {
    ...
    const larguraContainer = window.innerWidth;

    let personagemPosX = 0;
    let cenarioPosX = 0;
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (key === 'ArrowRight') {
            personagemPosX += 10;
            if (personagemPosX > larguraContainer / 2 && cenarioPosX > -(larguraCenario - larguraContainer)) {
                cenarioPosX -= 10;
                cenario.style.transform = `translateX(${cenarioPosX}px)`;
            } else {
                personagem.style.transform = `translateX(${personagemPosX}px)`;
            }
        } else if (key === 'ArrowLeft') {
            if (personagemPosX > 0) {
                personagemPosX -= 10;
                if (personagemPosX < larguraContainer / 2 && cenarioPosX < 0) {
                    cenarioPosX += 10;
                    cenario.style.transform = `translateX(${cenarioPosX}px)`;
                } else {
                    personagem.style.transform = `translateX(${personagemPosX}px)`;
...
const audioJogoIniciado = document.getElementById("audioJogoIniciado");
...
function moverPersonagem() {
  ...
  cenario.style.left = -posicao + larguraContainer / 2 - larguraPersonagem / 2 + "px"; // Move o cenário junto com o personagem
}
...
function colisaoComInimigo(){ // essa função deverá trocar depois, pq eu quero que o usario perca vidas e não tenha que recarregar a página
  const checarPersonagem = personagem.getBoundingClientRect();   // e que o inimigo nãos suma e continue andando
